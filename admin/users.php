...
if ($stmt = $conn->prepare("SELECT * FROM application_users WHERE AppUser_Role!=? ORDER BY AppUser_ID DESC")) 
{
...
?>
            ...
            <div class="row">
              <div class="col-md-12">
                <ul class="nav nav-pills">
                  <li class="active"><a href="users.php">User List <span class="badge"><?php echo $total_users?></span></a></li>
                </ul>          
              </div>
            </div>
          </div>
	...
	{
		$avatar=$getinfo['AppUser_Photo'];
		$UniqueID=$getinfo['AppUser_UniqueID'];
		$firstname=$getinfo['AppUser_Firstname'];
		$lastname=$getinfo['AppUser_Lastname'];
		$email=$getinfo['AppUser_Email'];
		$phone=$getinfo['AppUser_Phone'];
